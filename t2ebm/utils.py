...
def _try_api_resources_fallback(model, messages, **kwargs):
    ...
    try:
        ...
        raise OpenAICompletionError(
            f"Could not complete request with any OpenAI SDK method: {str(e)}"
        )
...
def create_direct_client(api_key=None):
    ...
    try:
        ...
        if not api_key:
            raise OpenAIInitializationError(
                "API key is required. Provide as parameter or set OPENAI_API_KEY environment variable."
            )
        ...
        if v1_api_available:
            try:
                ...
                try:
                    ...
                    raise OpenAIInitializationError(
                        f"Failed to create OpenAI client: {str(e2)}"
                    )
        else:
            # For older versions
