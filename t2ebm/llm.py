...
class LocalLLMChatModel(AbstractChatModel):
        ...
        try:
            if self.api_type == "ollama":
                ...
                if response.status_code != 200:
                    raise LocalLLMError(
                        f"Failed to connect to Ollama API at {self.base_url}: HTTP {response.status_code}"
                    )
            else:
                ...
                if response.status_code >= 500:
                    raise LocalLLMError(
                        f"Failed to connect to LLM API at {self.base_url}: HTTP {response.status_code}"
                    )
        except requests.exceptions.RequestException as e:
            raise LocalLLMError(
                f"Failed to connect to LLM API at {self.base_url}: {str(e)}"
            )
    ...
    def _ollama_completion(
        ...
        raise LocalLLMError(
            f"Unexpected response format from Ollama API: {response_data}"
        )
    ...
    def _generic_completion(
        ...
        try:
            ...
            raise LocalLLMError(
                f"Unexpected response format from LLM API: {response_data}"
            )
    ...
    def __init__(self, client, model: str):
        ...
        if client is None:
            raise OpenAIInitializationError(
                "OpenAI client is not initialized. Check your API key and environment."
            )
        self.client = client
        self.model = model
...
def create_openai_client(azure: bool = False) -> Optional[Any]:
    ...
    if OPENAI_INIT_ATTEMPTED and OPENAI_INIT_FAILED:
        raise OpenAIInitializationError(
            "OpenAI client initialization has already failed."
        )
    ...
    try:
        if azure:
            ...
            if not azure_endpoint or not azure_key:
                OPENAI_INIT_FAILED = True
                raise OpenAIInitializationError(
                    "AZURE_OPENAI_ENDPOINT or AZURE_OPENAI_KEY environment variables are not set."
                )
...
def chat_completion(
    llm: Union[str, AbstractChatModel, Dict[str, Any]], messages: List[Dict[str, Any]]
) -> List[Dict[str, Any]]:
    """
